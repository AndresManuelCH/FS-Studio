...
public class CVOASearch implements Callable<Individual> {
    ...
    //Evaluate of Weka
    private static SubsetEvaluator evaluador;
    //Debug
    private static boolean debuguear;
    //Patient zero
    private static int[] patientZero;

    public CVOASearch(int size, int max_time, String id, int seed, int minSpread, int maxSpread, int minSuperSpread,
    ...
    public CVOASearch(int size, int max_time, String id, int seed) {
        initializeCommon(size, max_time, id, seed);
    }
    ...
    public static void initializePandemic(Individual best, SubsetEvaluator ASEval, boolean debug, int[] pZero) {
    ...
    public static double fitness(Individual individual) throws Exception {
        return evaluador.evaluateSubset(transformador(individual.getData()));
    }
    
    public static BitSet transformador(int[] data) {
        BitSet aux = new BitSet(data.length);
    ...
    protected Individual infectPZ() throws Exception {
    ...
    //Transform the initial subset if provided
    protected Individual transformPZ(int[] pZero) throws Exception{
        Individual PZ = new Individual();
        int[] aux = new int[size];

        for(int i = 0; i < pZero.length; i++){
            aux[pZero[i]] = 1;
        }
